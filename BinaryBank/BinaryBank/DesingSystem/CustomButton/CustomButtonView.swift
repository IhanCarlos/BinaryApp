...
class CustomButton: UIButton {
    ...
    private func setupButton(style: Style) {
        ...
        case.containedLight:
            ...
            setTitleColor(.ds(.darkBlue), for: .normal)
            
        case.borderButton:
            layer.borderWidth = 1
            layer.borderColor = UIColor.ds(.white).cgColor
            clipsToBounds = true
            layer.cornerRadius = 25
            titleLabel?.font = .dsFonts(.poppinsBold14)
            setTitleColor(UIColor.ds(.white), for: .normal)
            backgroundColor = .clear
            
        case .containedGray:
            titleLabel?.font = .dsFonts(.poppinsRegular12)
